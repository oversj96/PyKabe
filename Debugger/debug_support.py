...
def compare_files(file1, file2, row_length):
    ...
    offset = 0
    with open("Debugger\\larger_file_differences.txt", "w") as file1, open(
        "Debugger\\smaller_file_differences.txt", "w"
    ) as file2:
        ...
        max_puzzle = int(
            larger_file[len(larger_file) - row_length - 2]
            .rstrip(".")
            .rstrip("\n")
        )
        puzzle = 0
        while puzzle <= max_puzzle:
            ...
            number1, rows1 = read_matrix(
                larger_file, increment_one, row_length
            )
            ...
            if mat1 != None:
                if mat1.first_water_point != None:
                    mat1.test_puzzle(
                        mat1.first_water_point[0], mat1.first_water_point[1], 1
                    )
