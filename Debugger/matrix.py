class Matrix:
    def __init__(self, row_length, number, rows, problem_rows, from_file):
        ...
        self.travel_map = [
            [True for j in range(0, row_length)] for i in range(0, row_length)
        ]
        self.water_count = 0
    ...
    def test_puzzle(self, pos_x, pos_y, call):
        ...
        for point in self.pos:
            x, y = point
            if (
                self.can_move(pos_x, x)
                and self.can_move(pos_y, y)
                and self.travel_map[pos_x + x][pos_y + y]
            ):
                self.test_puzzle(pos_x + x, pos_y + y, call + 1)
        if call == 1:
            self.problem_type = (
                "Illegal Puzzle"
                if self.water_reachable != self.water_count
                else "Missing Puzzle"
            )
